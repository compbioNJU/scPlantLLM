    ...
    def plot_metagenes_scores(self, adata, metagenes, column, plot=None):
        ...
        for cluster, vector in metagenes.items():
            ...
            for ct in set(adata.obs[column]):
                ...
                cts.append(ct)
            matrix.append(row)
            label = str(cluster) + "_SCORE: " + ", ".join(vector[:5])
            if len(set(vector)) > 5:
                label += "*"
    ...
    def score_metagenes(self, adata, metagenes):
        for p, genes in metagenes.items():
            try:
                ...
                if not genes:
                    raise ValueError(f"No valid genes found for {p}")

                # 计算元基因得分
                sc.tl.score_genes(adata, score_name=str(p) + "_SCORE", gene_list=genes)

                # 检查得分中是否有NaN或无穷大值
                scores = np.array(adata.obs[str(p) + "_SCORE"].tolist())
                if np.any(np.isnan(scores)) or np.any(np.isinf(scores)):
                    raise ValueError(f"Invalid score values found for {p}")
                ...
                # 检查归一化后的得分中是否有NaN或无穷大值
                if np.any(np.isnan(scores)) or np.any(np.isinf(scores)):
                    raise ValueError(f"Invalid normalized score values found for {p}")
